...
with st.sidebar:
    ...
    Delta_P = st.number_input("Diff Pressure ΔP [Pa]", value=1.8) # Needed for Flux Q
...
# --- Helper to create the 8-parameter table ---
def show_results_table(L_mm, D_mm, N):
    L_m = L_mm * 1e-3
    D_m = D_mm * 1e-3
    r_m = D_m / 2
    ...
    data = {
        ...
        "Diameter [mm]": [D_mm],
        "Numbers": [int(N)],
        "Total Volume [m³]": [f"{V_total:.2e}"],
        "C_trans Total [m³/s]": [f"{C_total:.4e}"],
        "Total Gas Flux Q": [f"{Q:.2e}"],
        "Residual Time [ms]": [f"{Res_Time_ms:.2f}"],
        "Overall Cond.": [f"{C_total:.5f}"]
    }
    df = pd.DataFrame(data)
    
    # Display
    st.success(f"Result: Overall Conductance = {C_total:.5f} m³/s")
    st.table(df) # Use st.table for a clean look
...
with tab2:
    ...
    if st.button("Calculate", key="btn2"):
        ...
        show_results_table(L2, D2, int(round(N2)))
...
with tab3:
    ...
    if st.button("Calculate", key="btn3"):
        ...
        try:
            ...
            show_results_table(L3, ans*1000, N3)
        except:
...
with tab4:
    ...
    if st.button("Calculate", key="btn4"):
        ...
        try:
            ...
            show_results_table(ans*1000, D4, N4)
        except:
