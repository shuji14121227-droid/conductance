...
if input_mode in ["1. Manual (t & D)", "2. Direct AR Input"]:
    if input_mode == "1. Manual (t & D)":
        ...
        # 🌟 step=0.01 に変更して小数の微調整を可能に
        final_AR = st.sidebar.number_input("Aspect Ratio (AR)", value=0.40, min_value=0.00, step=0.01)
        st.sidebar.info(f"Using AR = {final_AR:.2f}")
    ...
    ar_range = np.linspace(0.0, 5.0, 100) # ARが低い領域を見やすく調整
    ...
    fig.add_trace(go.Scatter(x=[final_AR], y=[leakage_rate], mode='markers+text', name='Current Point', text=[f"AR={final_AR:.2f}"], textposition="top right", marker=dict(color='red', size=12)))
...
# ==========================================
    ...
    t_opt = st.sidebar.number_input("Fixed Thickness (t) [mm]", value=1.0, step=0.1)
    C_target = st.sidebar.number_input("Target Conductance [m³/s]", value=0.0156368, format="%.7f")
    opt_variable = st.sidebar.radio("Vary Parameter by:", ["Aspect Ratio (AR)", "Hole Diameter (D)"])
    
    if opt_variable == "Aspect Ratio (AR)":
        # 🌟 step=0.01 に変更、初期値を0.40に
        ar_test = st.sidebar.slider("Test Aspect Ratio (AR)", 0.00, 5.00, 0.40, step=0.01)
        D_test = t_opt / ar_test if ar_test > 0 else float('inf')
    else:
        D_test = st.sidebar.slider("Test Hole Diameter (D) [mm]", 0.5, 20.0, 2.5, step=0.1)
    ...
    # 🌟 85%を超えているかどうかの色分けを追加
    if curr_R > 85.0:
        c4.metric("Single Hole Leakage", f"{curr_R:.2f} %", "Passes >85% Requirement")
    else:
        c4.metric("Single Hole Leakage", f"{curr_R:.2f} %", "- Fails >85% Requirement", delta_color="inverse")
    ...
    x_array = np.linspace(0.01, 5.0, 100) if opt_variable == "Aspect Ratio (AR)" else np.linspace(0.5, 20.0, 100)
    ...
    # 🌟 85%のターゲットライン（緑色の点線）を追加
    fig.add_hline(y=85.0, line_dash="dot", line_color="green", annotation_text="Target: >85%", secondary_y=False)
...
# ==========================================
    ...
    if fixed_param == "Fixed Thickness (t)":
        fixed_val = st.sidebar.number_input("Thickness (t) [mm]", value=1.0, step=0.1)
    else:
    ...
    ar_array = np.linspace(0.05, 5.0, 100)
